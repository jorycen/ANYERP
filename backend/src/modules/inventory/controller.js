...
async function buildSalesCountMap(productIds, storeId = '', scopedStoreIds = []) {
  ...
  const orderGrossProfitRows = await sequelize.query(
    `SELECT target.PRODUCT_ID AS product_id,
            SUM(CASE WHEN o.CREATE_TIME >= :date7 THEN all_items.SUBTOTAL ELSE 0 END) AS order_sales_7_amount,
       SUM(CASE WHEN o.CREATE_TIME >= :date7
                     THEN COALESCE(all_items.SALES_GROSS_PROFIT, all_items.SUBTOTAL - all_items.SALES_SETTLEMENT_COST * all_items.QUANTITY)
                     ELSE 0 END) AS order_gross_profit_7,
            COUNT(DISTINCT CASE WHEN o.CREATE_TIME >= :date7 THEN target.ORDER_ID END) AS order_count_7
       FROM (
  ...
  rows.forEach(row => {
    ...
    salesMap[row.product_id] = {
      ...
      avg_gross_profit_7: orderCount7 > 0 ? grossProfit7 / orderCount7 : 0,
  ...
  try {
    ...
    const sortedRows = products.map(p => {
      ...
      const sales = salesMap[p.product_id] || {
        sales_7_qty: 0, sales_30_qty: 0, sales_7_amount: 0, gross_profit_7: 0, avg_gross_profit_7: 0, gross_margin_7: 0
      };
      return {
