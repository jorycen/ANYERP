...
function getSettlementItemAvailableAmount(item, allocatedAmount = 0, adjustmentQuantityDelta = 0) {
  const adjustedQuantity = Math.max(
    0,
    Number(item?.quantity || 0) + Number(adjustmentQuantityDelta || 0)
  );
  const lineAmount = roundAmount(adjustedQuantity * actualUnitPrice(item));
  return roundAmount(Math.max(0, lineAmount - Number(allocatedAmount || 0)));
}
