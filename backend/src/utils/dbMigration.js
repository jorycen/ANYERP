...
async function ensureProductDimensionSchema() {
  await checkAndAddColumn(
    'T_PRODUCT',
    'CATEGORY_ID',
    'VARCHAR(32) NULL COMMENT "选定的商品分类节点"',
    'CATEGORY'
  );
  await checkAndAddColumn(
    'T_PRODUCT',
    'CATEGORY_PATH_LEGACY',
    'VARCHAR(512) NULL COMMENT "历史分类路径（兼容字段）"',
    'CATEGORY_ID'
  );
  await checkAndAddColumn(
...
// business facts.
async function ensureApprovalSchemaCompatibility() {
...
// 不生成产品端结算单、不回填返利，也不改写任何历史销售或财务事实。
async function ensureProductSettlementFeatureSchema() {
  ...
  await checkAndCreateTable('T_MANUFACTURER_REBATE_POLICY', `
    CREATE TABLE T_MANUFACTURER_REBATE_POLICY (
      ...
      POLICY_TYPE VARCHAR(32) DEFAULT 'activity',
      PRODUCT_ID VARCHAR(32),
      ...
      START_DATE DATE,
      END_DATE DATE,
      REBATE_CALCULATION_TYPE VARCHAR(32) DEFAULT 'fixed_amount',
      REBATE_AMOUNT DECIMAL(12,2) DEFAULT 0,
      REBATE_RATE DECIMAL(8,4) DEFAULT 0,
      AFFECT_SALES_SETTLEMENT_COST TINYINT(1) DEFAULT 0,
      COST_ADJUSTMENT_TYPE VARCHAR(32) DEFAULT 'fixed_amount',
      COST_ADJUSTMENT_VALUE DECIMAL(12,4) DEFAULT 0,
      MAX_COST_ADJUSTMENT_AMOUNT DECIMAL(12,2),
      COST_ADJUSTMENT_REMARK VARCHAR(512),
      REMARK VARCHAR(512),
  ...
  await checkAndCreateTable('T_REBATE_ESTIMATE', `
    CREATE TABLE T_REBATE_ESTIMATE (
      ESTIMATE_ID VARCHAR(32) NOT NULL,
      DISTRIBUTOR_ID VARCHAR(32),
      SALES_ORDER_ID VARCHAR(32) NOT NULL,
      SALES_ORDER_NO VARCHAR(64),
      SALES_ORDER_ITEM_ID BIGINT(20),
      SUPPLIER_ID VARCHAR(32),
      SUPPLIER_NAME VARCHAR(255),
      PRODUCT_ID VARCHAR(32),
      PRODUCT_NAME VARCHAR(255),
      ...
      REBATE_ESTIMATE_AMOUNT DECIMAL(12,2) DEFAULT 0,
  ...
  `);
  await checkAndAddColumn('T_REBATE_ESTIMATE', 'DISTRIBUTOR_ID', 'VARCHAR(32)', 'ESTIMATE_ID');
  ...
  await checkAndCreateTable('T_SALES_SETTLEMENT_COST_ADJUSTMENT', `
    CREATE TABLE T_SALES_SETTLEMENT_COST_ADJUSTMENT (
      ...
      PN VARCHAR(64),
      SN VARCHAR(128),
      ORIGINAL_INVENTORY_COST DECIMAL(12,2) DEFAULT 0,
      ORIGINAL_PICKUP_PRICE DECIMAL(12,2) DEFAULT 0,
      CURRENT_PICKUP_PRICE_AT_SALE DECIMAL(12,2) DEFAULT 0,
      POLICY_ID VARCHAR(32),
      POLICY_NAME VARCHAR(128),
      POLICY_TYPE VARCHAR(32),
      REBATE_ESTIMATE_ID VARCHAR(32),
      REBATE_ESTIMATE_AMOUNT DECIMAL(12,2) DEFAULT 0,
      AFFECT_SALES_SETTLEMENT_COST TINYINT(1) DEFAULT 0,
      COST_ADJUSTMENT_AMOUNT DECIMAL(12,2) DEFAULT 0,
      FINAL_SALES_SETTLEMENT_COST DECIMAL(12,2) DEFAULT 0,
      REMARK VARCHAR(512),
      CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      PRIMARY KEY (ID),
...
async function ensurePurchaseInvoiceSchema() {
  await checkAndCreateTable('T_PURCHASE_INVOICE', `CREATE TABLE T_PURCHASE_INVOICE (INVOICE_ID VARCHAR(32) NOT NULL PRIMARY KEY, DISTRIBUTOR_ID VARCHAR(32) NOT NULL, SUPPLIER_NAME VARCHAR(255), SUPPLIER_TAX_NO VARCHAR(64) NOT NULL, INVOICE_CODE VARCHAR(64) NOT NULL DEFAULT '', INVOICE_NO VARCHAR(64) NOT NULL, INVOICE_DATE DATE NOT NULL, INVOICE_TYPE VARCHAR(64) NOT NULL, TAX_RATE DECIMAL(6,4), AMOUNT_WITHOUT_TAX DECIMAL(12,2) NOT NULL, TAX_AMOUNT DECIMAL(12,2) NOT NULL, TOTAL_AMOUNT DECIMAL(12,2) NOT NULL, STATUS VARCHAR(32) NOT NULL DEFAULT 'active', REMARK VARCHAR(512), CREATE_USER VARCHAR(64), CREATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY uk_purchase_invoice_no (SUPPLIER_TAX_NO, INVOICE_CODE, INVOICE_NO), KEY idx_purchase_invoice_distributor (DISTRIBUTOR_ID, INVOICE_DATE)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='采购进项发票台账'`);
  ...
  const menuId = 'finance_purchase_invoice';
  await sequelize.query(`INSERT IGNORE INTO T_MENU (MENU_ID, MENU_CODE, NAME, PARENT_ID, MENU_TYPE, PATH, ICON, SORT_ORDER, STATUS) VALUES (?, ?, '发票管理', (SELECT MENU_ID FROM (SELECT MENU_ID FROM T_MENU WHERE MENU_CODE = 'finance' LIMIT 1) parent_menu), 'menu', '/finance/purchase-invoice', 'Money', 7, 1)`, { replacements: [menuId, menuId] });
  await sequelize.query(`INSERT IGNORE INTO T_ROLE_MENU (ROLE_ID, MENU_ID) SELECT r.ROLE_ID, m.MENU_ID FROM T_ROLE r JOIN T_MENU m ON m.MENU_CODE = 'finance_purchase_invoice' WHERE r.ROLE_CODE IN ('boss', 'admin', 'finance') AND r.STATUS = 1`);
}
...
async function ensureCriticalSchemaCompatibility() {
  ...
  if (Number(column?.cnt || 0) !== 1) {
    throw new Error('数据库缺少必要字段：T_PRODUCT.IS_USED_PRODUCT');
  }
  ...
  try {
    ...
    for (const idx of indexes) {
      ...
      if (columns === 'PRODUCT_ID,STORE_ID') {
        ...
        console.log(`[数据库迁移] 已删除商品/门店唯一索引：${idx.INDEX_NAME}`);
      }
    }
    ...
    console.error(`[数据库迁移] 规范化库存库位索引失败：${error.message}`);
  }
}
...
async function runMigrations() {
  ...
  try {
    ...
    await checkAndAddColumn('T_PRODUCT_SN', 'TAX_TYPE', 'VARCHAR(32) DEFAULT "UNKNOWN" COMMENT "税务属性"', 'ORIGINAL_PICKUP_PRICE');
    ...
    `);
    // 入库业务状态不得由迁移或启动流程回写。部分收货的历史修复仅能由
    // scripts/repair-partial-purchase-inbounds.js 显式执行。
    ...
    await checkAndAddColumn('T_PURCHASE_REQUEST', 'PAYMENT_METHOD', 'VARCHAR(32) NOT NULL DEFAULT "COMPANY_CREDIT" COMMENT "支付方式"', 'INVOICE_TYPE');
    ...
    await checkAndAddColumn('T_INVENTORY_RESOURCE_RIGHT', 'EFFECTIVE_START', 'DATETIME COMMENT "权益生效开始时间"', 'SOURCE_INBOUND_ID');
    await checkAndAddColumn('T_INVENTORY_RESOURCE_RIGHT', 'EFFECTIVE_END', 'DATETIME COMMENT "权益生效结束时间"', 'EFFECTIVE_START');
    ...
    await checkAndAddColumn('T_PRODUCT_RESOURCE_COST_CONFIG', 'CALCULATION_TYPE', 'VARCHAR(32) DEFAULT "fixed_amount" COMMENT "计算方式"', 'COST_AMOUNT');
    ...
    await checkAndAddColumn('T_PRODUCT', 'MANUFACTURER_CODE', 'VARCHAR(512) COMMENT "厂商编码"', 'CONFIG');
    ...
    await checkAndCreateTable('T_INVENTORY_CONVERSION', `
      CREATE TABLE T_INVENTORY_CONVERSION (
        ...
        CONVERSION_TYPE VARCHAR(32) NOT NULL COMMENT '转换类型',
    ...
    await checkAndCreateTable('T_INVENTORY_CONVERSION_ITEM', `
      CREATE TABLE T_INVENTORY_CONVERSION_ITEM (
        ...
        LINE_ROLE VARCHAR(32) NOT NULL COMMENT '明细行角色',
    ...
    await checkAndAddColumn('T_RETURN_STOCK', 'STATUS', "VARCHAR(32) DEFAULT 'pending' COMMENT '处理状态'", 'REASON');
    ...
    await checkAndAddColumn('T_EXPENSE', 'SOURCE_TYPE', 'VARCHAR(32) DEFAULT "expense" COMMENT "来源类型"', 'REVIEW_TIME');
    ...
    await checkAndCreateTable('T_PAYABLE', `
      CREATE TABLE T_PAYABLE (
        ...
        STATUS VARCHAR(32) DEFAULT 'unpaid' COMMENT '付款状态',
        CREATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    await checkAndAddColumn('T_PAYABLE', 'PAYEE_TYPE', 'VARCHAR(32) DEFAULT "supplier" COMMENT "收款方类型"', 'REQUEST_NO');
    ...
    await checkAndAddColumn('T_PAYABLE', 'SOURCE_TYPE', 'VARCHAR(32) DEFAULT "purchase" COMMENT "来源类型"', 'PAYEE_NAME');
    ...
    await checkAndCreateTable('T_SETTLEMENT', `
      CREATE TABLE T_SETTLEMENT (
        ...
        PAYMENT_STATUS VARCHAR(32) DEFAULT 'unpaid' COMMENT '付款状态',
    ...
    await checkAndAddColumn('T_SETTLEMENT', 'SETTLEMENT_TYPE', 'VARCHAR(32) DEFAULT "supplier" COMMENT "结算类型"', 'SUPPLIER_NAME');
    await checkAndAddColumn('T_SETTLEMENT', 'PAYEE_TYPE', 'VARCHAR(32) DEFAULT "supplier" COMMENT "收款方类型"', 'SETTLEMENT_TYPE');
    ...
    await checkAndAddColumn('T_SETTLEMENT', 'TAX_STATUS', 'VARCHAR(32) DEFAULT "UNKNOWN" COMMENT "税务属性快照"', 'SOURCE_NO');
    try {
      await sequelize.query(`
    ...
    await checkAndAddColumn('T_SETTLEMENT', 'PAYMENT_STATUS', 'VARCHAR(32) DEFAULT "unpaid" COMMENT "付款状态"', 'STATUS');
    ...
    await checkAndCreateTable('T_SETTLEMENT_PAYMENT_BATCH', `
      CREATE TABLE T_SETTLEMENT_PAYMENT_BATCH (
        ...
        STATUS VARCHAR(32) DEFAULT 'active' COMMENT '处理状态',
        REMARK VARCHAR(512) COMMENT '备注',
    ...
    await checkAndCreateTable('T_REBATE_POSTING_ORDER', `
      CREATE TABLE T_REBATE_POSTING_ORDER (
        ...
        SUPPLIER_ID VARCHAR(32) COMMENT '供应商ID',
        SUPPLIER_NAME VARCHAR(255) COMMENT '供应商名称快照',
        SETTLEMENT_TYPE VARCHAR(32) DEFAULT 'supplier' COMMENT '结算类型',
        PAYEE_TYPE VARCHAR(32) DEFAULT 'supplier' COMMENT '收款方类型',
        ...
        STATUS VARCHAR(32) DEFAULT 'UNMATCHED' COMMENT '核销状态',
    ...
    await checkAndAddColumn('T_REBATE_POSTING_ORDER', 'SOURCE_TYPE', 'VARCHAR(32) COMMENT "来源类型"', 'PAYEE_NAME');
    await checkAndAddColumn('T_REBATE_POSTING_ORDER', 'SOURCE_ID', 'VARCHAR(64) COMMENT "来源业务ID"', 'SOURCE_TYPE');
    await checkAndAddColumn('T_REBATE_POSTING_ORDER', 'SOURCE_NO', 'VARCHAR(64) COMMENT "来源业务单号"', 'SOURCE_ID');
    ...
    await checkAndCreateTable('T_MANUFACTURER_REBATE_POLICY', `
      CREATE TABLE T_MANUFACTURER_REBATE_POLICY (
        ...
        REBATE_CALCULATION_TYPE VARCHAR(32) DEFAULT 'fixed_amount' COMMENT '返利计算方式',
        ...
        COST_ADJUSTMENT_TYPE VARCHAR(32) DEFAULT 'fixed_amount' COMMENT '成本调整方式',
    ...
    await checkAndCreateTable('T_REBATE_ESTIMATE', `
      CREATE TABLE T_REBATE_ESTIMATE (
        ...
        STATUS VARCHAR(32) DEFAULT 'estimated' COMMENT '返利预估状态',
    ...
    await checkAndCreateTable('T_PERFORMANCE_PROFIT_ADJUSTMENT', `
      CREATE TABLE T_PERFORMANCE_PROFIT_ADJUSTMENT (
        ...
        STATUS VARCHAR(32) DEFAULT 'pending_finance' COMMENT '审批状态',
    ...
    await checkAndAddColumn('T_PURCHASE_REQUEST_ITEM', 'REBATE_DEDUCTION', 'DECIMAL(12,2) DEFAULT 0.00 COMMENT "明细返利抵扣"', 'SUBTOTAL');
    ...
    await checkAndCreateTable('T_DEPOSIT_ORDER', `
      CREATE TABLE T_DEPOSIT_ORDER (
        ...
        REDEEMED_AMOUNT DECIMAL(12,2) NOT NULL DEFAULT 0 COMMENT '已核销金额',
        REFUNDED_AMOUNT DECIMAL(12,2) NOT NULL DEFAULT 0 COMMENT '已退款金额',
        STATUS VARCHAR(32) DEFAULT 'submitted' COMMENT '处理状态',
    ...
    await checkAndAddColumn('T_DEPOSIT_REDEMPTION', 'STATUS', 'VARCHAR(32) DEFAULT "active" COMMENT "处理状态"', 'AMOUNT');
    ...
    await checkAndCreateTable('T_DAILY_STATEMENT', `
      CREATE TABLE T_DAILY_STATEMENT (
        ...
        STATUS VARCHAR(32) DEFAULT 'pending' COMMENT '结算状态',
    ...
    await checkAndCreateTable('T_DAILY_STATEMENT_DETAIL', `
      CREATE TABLE T_DAILY_STATEMENT_DETAIL (
        ...
        BUSINESS_TYPE VARCHAR(32) DEFAULT 'sales_receipt' COMMENT '业务类型',
    ...
    `);
    await checkAndAddColumn('T_SETTLEMENT_ACCOUNT', 'ACCOUNT_TYPE', 'VARCHAR(32) DEFAULT "FUND" COMMENT "结算账户类型"', 'ACCOUNT_NUMBER');
    ...
    await checkAndAddColumn(
      ...
      'AMOUNT_TYPE',
      'VARCHAR(16) NOT NULL DEFAULT "increase" COMMENT "调整方向"',
      'AMOUNT'
    );
    ...
    await checkAndCreateTable('T_INVENTORY_BATCH_APPLICATION', `
      CREATE TABLE T_INVENTORY_BATCH_APPLICATION (
        ...
        STATUS VARCHAR(32) DEFAULT 'pending' COMMENT '执行状态',
    ...
    await checkAndCreateTable('T_INVENTORY_BATCH_APPLICATION_ITEM', `
      CREATE TABLE T_INVENTORY_BATCH_APPLICATION_ITEM (
        ...
        VALIDATION_STATUS VARCHAR(32) DEFAULT 'valid' COMMENT '校验状态',
    ...
    `);
    await checkAndAddColumn('T_INVENTORY_BATCH_APPLICATION_ITEM', 'EXECUTE_STATUS', 'VARCHAR(32) DEFAULT "pending" COMMENT "执行状态"', 'ERROR_MESSAGE');
    ...
    await checkAndCreateTable('T_NON_SN_INVENTORY_BATCH_RIGHT', `
      CREATE TABLE T_NON_SN_INVENTORY_BATCH_RIGHT (
        ...
        STATUS VARCHAR(32) DEFAULT 'AVAILABLE' COMMENT '权益状态',
        CREATE_TIME DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
        UPDATE_TIME DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
        REMARK VARCHAR(512) COMMENT '备注',
    ...
    await checkAndCreateTable('T_PRODUCT_IMPORT_TASK', `
      CREATE TABLE T_PRODUCT_IMPORT_TASK (
        ...
        STATUS VARCHAR(32) DEFAULT 'queued' COMMENT '任务状态',
    ...
    await checkAndCreateTable('T_PRODUCT_PRICE_IMPORT_BATCH', `
      CREATE TABLE T_PRODUCT_PRICE_IMPORT_BATCH (
        ...
        STATUS VARCHAR(32) DEFAULT 'effective' COMMENT '生效状态',
        REMARK VARCHAR(512) COMMENT '备注',
        CREATE_USER VARCHAR(64) COMMENT '导入人',
        CREATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '导入时间',
        PRIMARY KEY (BATCH_ID),
    ...
    await checkAndCreateTable('T_PRODUCT_PRICE_CHANGE_LOG', `
      CREATE TABLE T_PRODUCT_PRICE_CHANGE_LOG (
        ...
        SOURCE VARCHAR(32) DEFAULT 'import' COMMENT '来源类型',
        CHANGE_REASON VARCHAR(512) COMMENT '调价原因',
        REMARK VARCHAR(512) COMMENT '备注',
        STATUS VARCHAR(32) DEFAULT 'pending' COMMENT '生效状态',
        CREATE_USER VARCHAR(64) COMMENT '操作人',
        CREATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    await checkAndCreateTable('T_PRODUCT_CATEGORY_FIELD', `
      CREATE TABLE IF NOT EXISTS T_PRODUCT_CATEGORY_FIELD (
        ...
        FIELD_OPTIONS TEXT COMMENT '下拉选项（JSON数组）',
...
async function seedPermissionData() {
  try {
    ...
    const childMenus = [
      ...
      ['finance_account', '账户中心', 'finance', '/finance/account', 11],
      ['finance_settlement', '应付结算单管理', 'finance', '/finance/settlement', 12],
      ['finance_freight', '运费管理', 'finance', '/finance/freight', 13],
    ...
    await checkAndAddColumn('T_PURCHASE_REQUEST_ITEM', 'NEW_PRODUCT_PAYLOAD', 'TEXT COMMENT "二手商品完整建档信息JSON"', 'DIRECT_INBOUND_SN_CODE');
    ...
    await checkAndAddColumn('T_PURCHASE_REQUEST', 'EXPRESS_NO', 'VARCHAR(128) COMMENT "快递单号"', 'INVOICE_TYPE');
