...
test('customer operations on isolated MySQL: races, ownership, returns, rollback', { skip: !run }, async () => {
  ...
  try {
    ...
    await Core.BusinessActionLog.sync();
    ...
    app.use(async (ctx,next)=>{if(ctx.path.startsWith('/test-staff'))ctx.state.user={staffId:1,roles:[ctx.get('X-Test-Role') || 'boss'],accessibleDistributorIds:['test_dist'],accessibleStoreIds:['test_store']};await next();});
    ...
    // Boss can configure fulfillment in both cities without pooling accounts.
    await Core.Distributor.create({distributor_id:'test_cq',name:'重庆测试',status:1,is_deleted:0});
    await Core.Store.create({store_id:'cq_store',distributor_id:'test_cq',name:'重庆门店',status:1,is_deleted:0});
    await Core.Store.create({store_id:'cq_other',distributor_id:'test_cq',name:'未选择门店',status:1,is_deleted:0});
    const staffUrl=`http://127.0.0.1:${server.address().port}/test-staff`;
    const bossOptions=await (await fetch(staffUrl+'/options')).json();
    assert.equal(bossOptions.data.canChooseAllRewardStores,true);
    assert.ok(bossOptions.data.stores.some(s=>s.store_id==='cq_store'));
    const adminOptions=await (await fetch(staffUrl+'/options',{headers:{'X-Test-Role':'admin'}})).json();
    assert.equal(adminOptions.data.canChooseAllRewardStores,false);
    assert.ok(!adminOptions.data.stores.some(s=>s.store_id==='cq_store'));
    const crossBody={distributor_id:'test_dist',name:'两地换购',kind:'gift',points:'300',cash_required:'39',stock:5,per_member_limit:null,valid_days:30,on_sale:true,store_ids:['test_store','cq_store']};
    const saveCross=(body,role='boss')=>fetch(staffUrl+'/rewards',{method:'POST',headers:{'Content-Type':'application/json','X-Test-Role':role},body:JSON.stringify(body)});
    assert.equal((await saveCross(crossBody,'admin')).status,403);
    assert.equal((await saveCross({...crossBody,store_ids:['missing_store']})).status,400);
    let savedCross=await saveCross(crossBody);const crossResponse=await savedCross.json();assert.equal(savedCross.status,200,JSON.stringify(crossResponse));
    const crossReward=crossResponse.data;
    const crossMember=await M.Member.create({phone:'13600000000'});
    await V.transaction(async t=>{const acc=await V.account(crossMember.id,'test_dist',t);await V.post(acc,1000n,{type:'activity',business_key:'cross-store-test'},t);});
    const crossDetail=(await (await get('/rewards/'+crossReward.id,S.sign(crossMember))).json()).data;
    assert.equal(crossDetail.stores.length,2);assert.equal(crossDetail.can_exchange,true);
    const crossExchange=await V.exchange(crossMember,crossReward.id,'cross_city_exchange01','300','39');
    assert.equal(crossExchange.snapshot.stores.find(s=>s.store_id==='cq_store').distributor_id,'test_cq');
    const cqStaff={staffId:1,roles:['staff'],accessibleDistributorIds:['test_cq'],accessibleStoreIds:['cq_store','cq_other']};
    const crossCode=S.token('redeem',crossExchange.id);
    await assert.rejects(V.redeem(crossCode,'cq_other',cqStaff,true,crossExchange.id),/不适用于本店/);
    await assert.rejects(V.redeem(crossCode,'cq_store',employee,true,crossExchange.id),/无权/);
    const crossRedeemed=await V.redeem(crossCode,'cq_store',cqStaff,true,crossExchange.id,{cash_confirmed:true,cash_received:'39',receipt_reference:'cq-sale-001'});
    assert.equal(crossRedeemed.redemption.store_id,'cq_store');
    assert.equal(crossRedeemed.redemption.distributor_id,'test_dist');
    assert.equal((await M.Account.findOne({where:{member_id:crossMember.id,distributor_id:'test_dist'}})).balance,'700');
    assert.equal(await M.Account.count({where:{member_id:crossMember.id,distributor_id:'test_cq'}}),0);
    const RS=require('../src/modules/customerOps/rewardStores');
    assert.equal(RS.accepts({storeIds:['cq_store']},{store_id:'cq_store',distributor_id:'test_cq'},'test_dist'),false);
    assert.equal(RS.accepts(crossExchange.snapshot,{store_id:'cq_store',distributor_id:'moved_dist'},'test_dist'),false);
    // A reward usable only in Chongqing remains visible to its Chengdu account.
    await M.RewardStore.destroy({where:{reward_id:crossReward.id,store_id:'test_store'}});
    const cqOnly=(await (await get('/rewards/'+crossReward.id,S.sign(crossMember))).json()).data;
    assert.equal(cqOnly.stores.length,1);assert.equal(cqOnly.can_exchange,true);
