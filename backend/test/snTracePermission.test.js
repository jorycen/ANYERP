...
test('SN追踪：经销商级账号仅查看本经销商单据', () => {
  assert.equal(canViewSnTraceReference({ roles: ['finance'], distributorId: 'dist-a' }, reference), true);
  assert.equal(canViewSnTraceReference({ roles: ['finance'], distributorId: 'dist-b' }, reference), false);
});
