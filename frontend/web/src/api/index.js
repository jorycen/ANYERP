...
export default {
  ...
  getSalesApprovalList: (params = {}) => api.get('/sales/list', { params: { ...params, status: 'pending_approval', scope: 'review' } }),
  ...
  getSalesReturnRequests: (params) => api.get('/sales/return-requests', { params }),
  submitSalesReturnRequest: (data) => api.post('/sales/return-requests', data),
  reviewSalesReturn: (id, data) => api.post(`/sales/return-requests/${id}/review`, data),
  confirmSalesReturnRefund: (id, data) => api.post(`/sales/return-requests/${id}/refund-confirm`, data),
  ...
  importSupplierInventory: (file, vendor) => {
    const formData = new FormData()
    formData.append('file', file)
    formData.append('vendor', vendor)
    return api.post('/inventory/supplier-inventory/import', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },
  ...
  getExpenseAccountingPeriods: (params = {}) => api.get('/finance/expense-accounting-periods', { params }),
  closeExpenseAccountingPeriod: (monthKey, data = {}) => api.post(`/finance/expense-accounting-periods/${monthKey}/close`, data),
  reopenExpenseAccountingPeriod: (monthKey, data = {}) => api.post(`/finance/expense-accounting-periods/${monthKey}/reopen`, data),
  ...
  exportPurchaseInvoiceCandidates: () => exportExcel('/finance/purchase-invoices/candidates/export', {}, `未登记发票清单_${new Date().toISOString().slice(0, 10)}.xlsx`),
  ...
  importPurchaseInvoices: (rows) => api.post('/finance/purchase-invoices/import', { rows }),
  ...
  getProductApplicationDetail: (id) => api.get(`/product/application/${id}`),
